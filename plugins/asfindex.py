...
# get site menu
def get_menu(site_index, folders):
    currrent_menu = None
    site_menu = ''
    if folders:
        for f in folders:
...
def show_pages(generators):
    ...
    set_context(generators, 'SITE_MENU', get_menu(site_index, get_setting(generators, 'SITE_MENUS')))
    set_context(generators, 'SITE_INDEX', get_index(site_index))
